...
/**
 * Controller cho AI Permission Recommendation APIs
 * Proxy các request tới PoweredAI-RBAC service
 */
@RestController
...
public class AIController {
     ...
     * POST /api/ai/recommend/new-user
     * 
     * Request body:
     * {
     *   "role": "Doctor",
    ...
    public ResponseEntity<ApiResponse<Map<String, Object>>> recommendNewUser(
        ...
        String license = profile.getOrDefault("license", "No");
        String seniority = profile.getOrDefault("seniority", "Junior");
     ...
     * POST /api/ai/recommend/job-transfer
     * 
     * Request body:
     * {
     *   "old_profile": {
    ...
    public ResponseEntity<ApiResponse<Map<String, Object>>> recommendJobTransfer(
        ...
        if (oldProfile == null || newProfile == null) {
            return ResponseEntity.badRequest().body(ApiResponse.error("old_profile and new_profile are required"));
        }
     ...
     * POST /api/ai/recommend/rightsizing?lookbackDays=90
     */
     ...
     * POST /api/ai/recommend/anomaly?riskThreshold=3
     */
    ...
    /**
     * Kiểm tra trạng thái kết nối AI service
     * GET /api/ai/health
     */
    @GetMapping("/health")
